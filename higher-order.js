...
let totalPopulation = populations.reduce((sum, currentVal) => sum += currentVal)
...
const getSum = (obj, person) => {
  for (let i in obj) {
    if (obj[i].owner === person) {
      sum += obj[i].price;
    }
  }
  return sum;
...
getSum(purchases, `Bob`);

let bobsTotal = purchases.reduce(getSum, `Bob`);
